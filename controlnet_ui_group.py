...
class ControlNetUiGroup(object):
    ...
    def render(self, tabname: str, elem_id_tabname: str) -> None:
        ...
        """
        with gr.Tabs():
            with gr.Tab(label="תמונה בודדת") as self.upload_tab:
                with gr.Row(elem_classes=["cnet-image-row"]).style(equal_height=True):
                    ...
                    with gr.Group(
                        ...
                        self.generated_image = gr.Image(
                            label="תצוגה מקדימה של קדם מעבד",
            ...
            with gr.Tab(label="קבוצה") as self.batch_tab:
        ...
        with gr.Row(elem_classes="controlnet_image_controls"):
            gr.HTML(
                value="<p>הגדר את המעבד המקדים ל-[היפוך] אם לתמונה שלך יש רקע לבן וקווים שחורים.</p>",
        ...
        with FormRow(
            ...
            self.preprocessor_preview = gr.Checkbox(
                label="אפשר תצוגה מקדימה", value=False, elem_id=preview_check_elem_id
            )
            self.use_preview_as_input = gr.Checkbox(
                label="תצוגה מקדימה כקלט",
                value=False,
        ...
        self.loopback = gr.Checkbox(
            label="זו ControlNet שלח אוטומטית תמונות שנוצרו ליחידת [loopback]",
